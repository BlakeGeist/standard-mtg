class MechanicsController < ApplicationController
 ...
 def show
   ...
   @cards = Card.where('set=? OR set=? OR set=? OR set=? OR set=? OR set=? OR set=?', 'KLD', 'EMN', 'SOI', 'W16', 'OGW', 'BFZ', 'ORI', 'DTK').where("text like ? OR text like ?", "%#{@mechanic.name}%", "%#{@mechanic.name.downcase}%").sort_by{ |t| [t.cmc.to_i, t.colors] }.paginate(:page => params[:page], :per_page => 28)
 end
